/*
    quickPlayHero/consoleController.js
    处理玩家主机平台快速模式英雄信息
*/
...
async function getConsoleQuickPlayHeroInfo(req, res) {
    try {
        let {playerTag, heroID, refreshCache} = req.query;
        const heroName = heroesData.find(hero => hero.heroID.toString() === heroID).heroName;
        console.log(`${getCurrentTime()} Received console quickPlay hero info request with playerTag: \u001b[33m${playerTag}\u001b[0m, heroID: \u001b[33m${heroID}\u001b[0m, heroName: \u001b[33m${heroName}\u001b[0m.`);
        ...
        if (isPrivate) {
            ...
            console.log(`${getCurrentTime()} \u001b[33m${playerTag} (Private)\u001b[0m‘s console quickPlay hero data for heroID \u001b[33m${heroID}\u001b[0m has been scraped successfully.`);
            return res.status(200).json(isPrivateResult);
        }

        // 获取页面元素中的所有英雄信息
        const heroElements = $('body > main.main-content > div.controller-view.Profile-view > blz-section.stats.quickPlay-view > div.Profile-heroSummary--header > select[data-dropdown-id="hero-dropdown"] option')
            .map((index, element) => ({
        ...
        // 构建动态的选择器
        const heroSelector = `.controller-view.Profile-view blz-section.stats.quickPlay-view span.stats-container.option-${selectedHero.heroSourceID}`;

        // 提取指定位置的元素信息
        const quickHeroData = $(heroSelector).html();

        // 使用Cheerio解析元素信息
        $ = cheerio.load(quickHeroData);
        ...
        console.log(`${getCurrentTime()} \u001b[33m${playerTag}\u001b[0m‘s console quickPlay hero data for heroID \u001b[33m${heroID}\u001b[0m has been scraped successfully.`);
...
module.exports = {getConsoleQuickPlayHeroInfo};